...
document.addEventListener('DOMContentLoaded', function() {
    ...
    onAuthStateChanged(auth, (user) => {
        if (user) {
            ...
            userInfoDiv.innerHTML = `<span>${user.email}</span><button id="logout-button">로그아웃</button>`;
    ...
    if(signupButton) {
        signupButton.addEventListener('click', () => {
            ...
            createUserWithEmailAndPassword(auth, email, password)
                .then(() => alert('회원가입에 성공했습니다! 이제 로그인해주세요.'))
                .catch(error => alert(`회원가입 중 오류가 발생했습니다: ${error.message}`));
        });
    }
    ...
    function initializeAppForUser(user) {
        ...
        async function getStoredData(dateString) {
            const initialData = { score: STARTING_SCORE, checkedItems: {}, exerciseChecked: false, exerciseChoices: [], journalMemo: '', studyHours: 0 };
            return allUserData[dateString] || initialData;
        }

        async function saveData(dateString, data) {
            allUserData[dateString] = data;
            const userDocRef = doc(db, 'users', user.uid);
            await setDoc(userDocRef, { [dateString]: data }, { merge: true });
        }

        async function loadStateForDate(dateString) {
            const data = await getStoredData(dateString);
        ...
        async function updateScore() {
            ...
            await saveData(dateString, data);
            
            await renderCalendarScores();
        ...
        function addEventListeners() {
            document.querySelectorAll('input[type="checkbox"]').forEach(cb => {
                cb.addEventListener('change', async () => {
                    const data = await getStoredData(getDateString(selectedDate));
                    data.checkedItems[cb.id] = cb.checked;
                    await saveData(getDateString(selectedDate), data);
                    await updateScore();
                });
            });
            const mainExerciseBtn = document.getElementById('exercise-main-button');
            const choicesContainer = document.getElementById('exercise-choices');
            mainExerciseBtn.addEventListener('click', async () => {
                const data = await getStoredData(getDateString(selectedDate));
                data.exerciseChecked = !data.exerciseChecked;
                if (!data.exerciseChecked) { data.exerciseChoices = []; }
                await saveData(getDateString(selectedDate), data);
                await loadStateForDate(getDateString(selectedDate));
            });
            choicesContainer.querySelectorAll('button').forEach(btn => {
            ...
            journalTextarea.addEventListener('input', async () => {
                const data = await getStoredData(getDateString(selectedDate));
...
_            });
            const studySlider = document.getElementById('study-slider');
            if(studySlider){
                studySlider.addEventListener('input', async () => {
                    const data = await getStoredData(getDateString(selectedDate));
        ...
        async function renderCalendarScores() {
            for (const dayEl of document.querySelectorAll('.day[data-date]')) {
        ...
        async function getLast7DaysScores() {
        ...
        async function updateAnalytics() {
            ...
            const currentData = await getStoredData(getDateString(today));
            const currentScore = currentData.score;
            
            const monthStart = new Date(today.getFullYear(), today.getMonth(), 1);
            const monthEnd = new Date(today.getFullYear(), today.getMonth() + 1, 0);
            const monthlyAvg = await getAverageScore(monthStart, monthEnd);
            ...
            const weeklyAvg = await getAverageScore(weekStart, weekEnd);
            ...
            const { scores, labels } = await getLast7DaysScores();
        ...
        function renderCalendar() {
            ...
            for (let i = 1; i <= daysInMonth; i++) {
                ...
                dayEl.addEventListener('click', async () => { 
                    ...
                    await loadStateForDate(dateString);
                });
